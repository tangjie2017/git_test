    ...
    u.prototype.type = function() {
        ...
        u.prototype.view = function() {
            ...
            a.curr > a.pages && (a.curr = a.pages),
                ...
                a.prev = "prev"in a ? a.prev : $.getMessage('prev'),
                a.next = "next"in a ? a.next : $.getMessage('next');
            var n = a.pages > t ? Math.ceil((a.curr + (t > 1 ? 1 : 0)) / (t > 0 ? t : 1)) : 1
                ...
                page: function() {
                    ...
                    0 !== t && e.push('<a href="javascript:;" class="layui-laypage-last" title="'+$.getMessage('lastPage')+'"  data-page="' + a.pages + '">' + (a.last || a.pages) + "</a>")),
                        e.join("")
                }(),
                next: function() {
                    return a.next ? '<a href="javascript:;" class="layui-laypage-next' + (a.curr == a.pages ? " " + r : "") + '" data-page="' + (a.curr + 1) + '">' + a.next + "</a>" : ""
                }(),
                count: '<span class="layui-laypage-count"> '+$.getMessage('TotalPage') +' '+ a.count +' '+$.getMessage('Datas')+'</span>',
                limit: function() {
                    var e = ['<span class="layui-laypage-limits"><select lay-ignore>'];
                    return layui.each(a.limits, function(t, n) {
                        e.push('<option value="' + n + '"' + (n === a.limit ? "selected" : "") + ">" + n +  $.getMessage('Datas')+"/"+$.getMessage('Page')+"</option>")
                    }),
                    e.join("") + "</select></span>"
                }(),
                refresh: ['<a href="javascript:;" data-page="' + a.curr + '" class="layui-laypage-refresh">', '<i class="layui-icon layui-icon-refresh"></i>', "</a>"].join(""),
                skip: function() {
                    return ['<span class="layui-laypage-skip">'+$.getMessage('Goto'), '<input type="text" min="1" value="' + a.curr + '" class="layui-input">',
                        $.getMessage('Page')+' <button type="button" class="layui-laypage-btn">'+$.getMessage('PageConfirm')+'</button>', "</span>"].join("")
                }()
            };
