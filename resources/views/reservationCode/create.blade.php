...
<div class="bigbox"  style="background: #fff; padding: 20px; position: absolute; top: 50%;   left: 50%; margin-top: -320px; margin-left: -590px; border-radius: 5px; ">
    <div class="titile">
        ...
        <div class="english">
            <select name="language" class="language" onchange="changeLan('{{ session()->get('content') ?? '' }}'+'&id='+'{{  session()->get('yuyueid') ?? ''  }}')" style="border:0px solid #ccc;background: #f2f2f2;">
...
<script>
    ...
    layui.use(['form','laydate'], function(){
        ...
        //给a标签赋值
        document.getElementById("a").href="/reservation_code/index"+"?content="+"<?php echo Request::session()->get('content','')?>";
                ...
                success:function(response){
                    ...
                    if(response.Status){
                        ...
                        setTimeout(function(){  //使用  setTimeout（）方法设定定时2000毫秒
                            window.location.href="{{url('reservation_code/index')}}?content="+"<?php echo Request::session()->get('content', '')?>";
                        },2000);
