...
@section('content')
    ...
    <div class="pdabg yuyueh5">
        ...
        <h5>{{ __('auth.ReturningTheCounter') }}</h5>
        <div class="pdalogin yuyueform">
            <form action="" class="layui-form">
                ...
                <table class="layui-table layui-form" lay-skin="nob">
                    ...
                    <tr><td><input type="text" style="padding-left: 0; margin-top: 0; " name="reservationNum" class="reservationNum" maxlength="32" placeholder="{{ __('auth.SupportCabinetNumberReservationNumber') }}" autofocus  onkeypress="return inputNum(event)"></td></tr>
                    <tr><td>{{ __('auth.ActualReturnDate') }}：</td></tr>
                    <tr><td><input style="padding-left: 0; margin-top: 0;" type="text" name="actual_return_time" class="realTime" id="test10" placeholder="{{ __('auth.localCreateTime') }}"></td></tr>
                    <tr><td>{{ __('auth.file') }}：</td></tr>
                    <tr><td>
        ...
        layui.use([ 'jquery', 'layer', 'laydate','form', 'element','upload' ], function() {
            ...
            form.on('submit(searBtn)',function(data){
                ...
                if( !field.reservationNum){
                    layer.msg('{{ __('auth.SingleNumberRequired') }}',{icon :6})
                    return false;
                }
                if( !field.actual_return_time){
                    layer.msg('{{ __('auth.ActualReturnDateRequired') }}',{icon :6})
                    return false;
                }
                if(files.length == 0){
                ...
                if(files.length > 5){
                    layer.msg('{{ __('auth.UploadTo5Images') }}',{icon :6})
                    return false;
                }
                $.MXAjax({
                    type: "post",
                    url: "/pda/cabinet_submit",
                    dataType:"json",
                    data:{
                        '_token':"{{csrf_token()}}" ,
                        'info': data.field,
